  ...
  useEffect(() => {
    if (currentUser) {
      if (!currentUser.isEmailVerified) {
        navigate(`/check-email/${encodeURIComponent(currentUser.email)}`);
      } else if (!currentUser.company) {
        navigate("/setup-workspace");
      } else if (!currentUser.setupCompleted && currentUser.isAdmin) {
        navigate("/setup-workspace");
      }
    }
  ...
  return (
    <div className="flex min-h-screen bg-[var(--color-bg-light)] dark:bg-[var(--color-bg-dark)] text-gray-900 dark:text-white font-geist transition-all duration-700 ease-in-out overflow-x-hidden">
      ...
      <main className="flex-1 min-w-0 min-h-screen transition-all duration-700 lg:ml-72">
        ...
        >
          {activeTab === "Dashboard" ? (
            <MainDashboardView onSeeAllProjects={() => setActiveTab("My Projects")} currentUser={currentUser} projects={projects} />
...
const MainDashboardView = ({ onSeeAllProjects, currentUser, projects = [] }) => {
  const displayName = currentUser?.fullName?.split(" ")[0] || currentUser?.username || "User";
  
  // Calculate dynamic stats
  const activeProjectsCount = projects.length;
  
  const averageProgress = projects.length > 0
    ? Math.round(projects.reduce((acc, item) => {
        const p = item.project || item;
        return acc + (p.progress || 0);
      }, 0) / projects.length)
    : 0;

  // Map first 3 projects to Recent Projects section
  const recentProjects = projects.slice(0, 3).map((item, index) => {
    const p = item.project || item;
    ...
    return {
      ...
      icon: index % 3 === 0 ? "cloud" : index % 3 === 1 ? "hub" : "security",
      iconBg: index % 3 === 0 ? "bg-blue-500/10" : index % 3 === 1 ? "bg-purple-500/10" : "bg-emerald-500/10",
      iconColor: index % 3 === 0 ? "text-blue-500" : index % 3 === 1 ? "text-purple-500" : "text-emerald-500"
    };
  });
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="flex items-center gap-3 bg-blue-500/5 border border-blue-500/10 p-4 rounded-2xl w-fit">
          ...
          <p className="text-sm font-medium text-blue-600 dark:text-blue-400">
            {activeProjectsCount > 0 
              ? `You have ${activeProjectsCount} active project${activeProjectsCount > 1 ? 's' : ''} in your workspace today.`
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Active Projects" value={activeProjectsCount.toString()} subValue="Real-time count" subColor="text-emerald-500" />
        <StatCard label="Total Tasks" value="4" subValue="Across all sprints" subColor="text-gray-400" />
        <StatCard label="Team Members" value="1" hasTrend icon="trending_up" />
        <StatCard label="Overall Progress" value={`${averageProgress}%`} isProgress progress={averageProgress} icon="analytics" />
      </div>
        ...
        <div className="xl:col-span-2 space-y-10">
          <section>
            ...
            {recentProjects.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-gray-400 border border-dashed border-gray-100 dark:border-white/5 rounded-3xl">
                <span className="material-symbols-outlined text-4xl mb-2">folder_open</span>
                <p className="text-xs font-bold">No projects created yet.</p>
              </div>
            )}
          </section>
